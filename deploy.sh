...
for folder in $CHANGED_FOLDERS; do
  ...
  if [ -f "$folder/template.yaml" ]; then
    ...
    SAM_CLI_POLL_DELAY=5 sam deploy --stack-name "$folder-sam-stack" --s3-bucket $Bucket --s3-prefix $folder-sam-stack --capabilities CAPABILITY_IAM CAPABILITY_NAMED_IAM
  else
    ...
    if [ -f "$folder/domain.yaml" ]; then
      ...
      SAM_CLI_POLL_DELAY=5 sam deploy --stack-name "$folder-domain-sam-stack" --s3-bucket $Bucket --s3-prefix $folder-domain-sam-stack --capabilities CAPABILITY_IAM CAPABILITY_NAMED_IAM
    else
